...
/*
    ...
    Allowed numbers, only one per line: spaces, +/-, any number of zeros,
   number, spaces |number| < 1000000000
*/
...
class Solution {
  ...
  public:
    ...
    class iterator {
      ...
      public:
        iterator(std::istream& s, std::streampos pos, bool finished)
            : m_src(s), m_pos(pos), m_n(0), m_finished(finished) {}

        bool operator==(const iterator& other) const {
            return (m_finished == other.m_finished) &&
                   (m_finished || (m_pos == other.m_pos));
        }
        bool operator!=(const iterator& other) const {
      ...
      private:
        //  no assignment operator, but the generated copy constructor is
        //  necessary
...
void Solution::iterator::readNextLine(std::string& line) {
    ...
    //  skip new line
    if (!m_src.eof()) m_src.seekg(1, std::ios_base::cur);
    m_pos = m_src.tellg();
...
bool Solution::iterator::getInteger(const std::string& line, int& n) {
    ...
    if (negative || (line[i] == '+')) ++i;
    // eat zeros
    for (; (i < line.length()) && ('0' == line[i]); ++i)
        ;
    if (i == line.length()) {
        if ((i == 0) || ('0' != line[i - 1])) return false;
        n = 0;
        return true;
    }
    //  number
    size_t j;
    for (j = i; (j < line.length()) && ('0' <= line[j]) && ('9' >= line[j]);
         ++j)
        ;
    //  trim
    size_t k;
...
int Solution::iterator::operator*() const {
    if (m_finished) throw std::overflow_error("end stream passed");
    return m_n;
}
