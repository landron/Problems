...
static inline bool Trim(wchar_t c) {
    switch (c) {
    case L' ':
    case L'\t':
    case L'\r':
...
static size_t Trim(const wchar_t* src, size_t len) {
    assert(src && len);
    size_t i;
...
static inline bool GetString(const wchar_t* src, size_t len, JValue& value) {
    assert(src && len);
    assert(L'"' == *src);
    size_t i;
    for (i = 1; i < len; ++i) {
        if (L'"' == src[i] && ((i == 0) || (L'\\' != src[i - 1]))) break;
    }
...
static JResult GetValue(const wchar_t* src, size_t len, JValue& value) {
    JResult result;
    ...
    switch (src[next]) {
    case L'"': {
        if (!GetString(src + next, len - next, value)) return result;
    ...
    case L'f':
    case L'n': {
        if (0 == wcsncmp(src + next, L"true", 4) ||
            0 == wcsncmp(src + next, L"null", 4)) {
...
JResult GetArray(const wchar_t* src, size_t len, JValue& theArray) {
    ...
    for (next = 1; next < len;) {
        ...
        if (!temp.result) {
            if ((next < len) && (L']' == src[next]) && !foundValue) break;
            return result;
        }
        foundValue = true;
        ...
        if (next == len - i) return result;
        next += i;
        assert(next < len);
        if (L']' == src[next]) break;
        if (L',' != src[next]) return result;
...
JResult Object_GetNextItem(const wchar_t* src, size_t len, const size_t fromPos,
    ...
    JResult temp = GetValue(src + i, len - i, value);
    next = temp.next + i;
    if (!temp.result || (next == len)) return result;
    assert(next < len);
    value.start += i;
    ...
    if (len != next) {
        i = next;
        next = Trim(src + i, len - i);
        if (next == len - i) return result; // abrupt ending
        next += i;
    }
    assert(next <= len);
...
// ignore (ending) parsing error as we alreay checked them previously AND we are
// only interested in the values
void Object_GetNextItem_IgnoreParseErrors(const wchar_t* src, size_t len,
...
static bool GetJSON(std::wistream& src, std::wstring& jsonOut) {
    ...
    wchar_t buff[256];
    for (int i = stop - start; i > 0;
         i -= (sizeof(buff) / sizeof(wchar_t) - 1)) {
...
JsonItem::iterator JsonParser::begin() {
    std::wstring json;
    if (!GetJSON(m_src, json)) return end();

    //  validate
    JValue object;
    JResult validate = GetObject(json.c_str(), json.length(), object);
    if (!validate.result) return end();
...
JsonItem::iterator& JsonItem::iterator::operator++() {
    ...
    std::wstring name;
    JValue value;
    JResult result = Object_GetNextItem(m_src.c_str(), m_src.length(), m_pos,
                                        name, value, end);
    if (result.result) m_pos = result.next;
    else m_pos = m_src.length();
...
JsonItem JsonItem::iterator::operator*() const {
    ...
    JResult result = Object_GetNextItem(m_src.c_str(), m_src.length(), m_pos,
                                        name, value, end);
    if (!result.result) return JsonItem(L"", TYPE_UNDEFINED);
...
std::wstring JsonItem::Name() const {
    std::wstring name;
    JValue value;
    Object_GetNextItem_IgnoreParseErrors(m_src.c_str(), m_src.length(), 0, name,
                                         value);
