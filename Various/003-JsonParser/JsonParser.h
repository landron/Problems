...
/*
   ...
   2.  think of a version that works directly with istream
         for fun, as string is not very expensive since JSON is thought for not
   very large tansfers (I think)
*/
...
class JsonItem {
  ...
  public:
    JsonItem(const std::wstring& item, unsigned type)
        : m_src(item), m_type(type) {}
    ...
    class iterator {
      ...
      public:
        ...
        bool operator==(const iterator& other) const {
            return (m_src.length() == m_pos) ==
                   (other.m_src.length() == other.m_pos);
        }
        bool operator!=(const iterator& other) const {
