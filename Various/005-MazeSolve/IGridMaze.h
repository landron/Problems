...
/*
   x is the first coordinate, left to right; y is the second, from bottom to
   above
*/
class IGridMaze : public IMaze {
  ...
  public:
    ...
    unsigned Movements() const {
        return m_movements;
    }

  protected:
    const std::pair<unsigned, unsigned>& Mouse() const {
        return m_mouse;
    }
    bool MouseIs(unsigned x, unsigned y) const {
        return x == m_mouse.first && y == m_mouse.second;
    }

    const std::pair<unsigned, unsigned>& Cheese() const {
        return m_cheese;
    }
  ...
  private:
    // TODO: validate consistency = if right is possible, then left is possible
    // also for the opposite point
  ...
  public:
    ...
    virtual bool Allowed(Direction d) const {
        ...
        case right:
            return MouseIs(0, 1);
        case up:
            return 0 == Mouse().second;
        case down:
            return 1 == Mouse().second;
        };
...
class Maze2x2_2 : public Maze2x2_1<_hasCheese> {
  ...
  public:
    virtual bool Allowed(Direction d) const {
        switch (d) {
        case left:
            return 1 == Mouse().first;
        case right:
            return 0 == Mouse().first;
        }
