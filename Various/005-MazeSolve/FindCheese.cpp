...
static inline bool FindCheese(IMaze* maze,
...
bool FindCheese(IMaze* maze) {
    ...
    // logically, the maze should already be initialized BUT 1) as it was not
    // precised and the function is there, plus
    ...
    // "The maze will be no larger than 100 by 100 units" => allow 100 units in
    // any direction
    bool visited[2 * LIMIT_MAZE][2 * LIMIT_MAZE] = {};
...
static bool FindCheese_Maze(IMaze* maze,
                            bool visited[2 * LIMIT_MAZE][2 * LIMIT_MAZE],
                            ValueType& current) {
    ...
    for (int i = 0; i < sizeof(directions) / sizeof(directions[0]); ++i) {
        ...
        case up:
            ...
            break;
        case right:
            if (x < (2 * LIMIT_MAZE - 1) && !visited[x + 1][y] &&
                maze->Move(right)) {
                d = right;
                ++x;
            }
            break;
        case down:
            if (y < (2 * LIMIT_MAZE - 1) && !visited[x][y + 1] &&
                maze->Move(down)) {
                d = down;
                ++y;
            }
        ...
        visited[x][y] = true;
        if (maze->Success()) return true;
        if (FindCheese(maze, visited, current)) return true;
...
static bool FindCheese_Grid(IMaze* maze,
                            bool visited[2 * LIMIT_MAZE][2 * LIMIT_MAZE],
                            ValueType& current) {
    ValueType::first_type& x = current.first;
    ValueType::second_type& y = current.second;
...
bool FindCheese(IMaze* maze, bool visited[2 * LIMIT_MAZE][2 * LIMIT_MAZE],
                ValueType& current) {
