...
uint32_t get_distinct_by_prev_validation(uint32_t N, uint32_t S, uint32_t P,
                                         uint32_t Q) {
    uint32_t length = 0;
    for (uint32_t curr = S, prev = curr - 1; prev != curr && length < N;
         prev = curr, curr = P * curr + Q, ++length)
        ;
    return length;
}

uint32_t get_distinct_by_Floyd(uint32_t N, uint32_t S, uint32_t P, uint32_t Q,
                               bool print = false) {
...
void debug_validations() {
    ...
    assert(10000000 ==
           get_distinct(10000000, 658061970, 695098531, 1430548937));

    // good anwsers
    assert(4 == get_distinct_by_prev_validation(1000, 1, 1024, 9));
    assert(32 == get_distinct_by_prev_validation(100000000, 569099406,
                                                 1607140150, 823906344));
    assert(27 == get_distinct_by_prev_validation(100000000, 1232077670,
                                                 126810854, 1536183938));
    assert(10000000 == get_distinct_by_prev_validation(10000000, 658061970,
                                                       695098531, 1430548937));
    // bad answers
