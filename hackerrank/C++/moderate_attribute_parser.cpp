...
inline static size_t find_close(const string& text, size_t start,
                                const string& tag) {
    string tagged = "</";
    tagged += tag;
    tagged += '>';
...
inline static bool update_tag(const string& text, size_t start,
                              const string& tag, string& lasttag) {
    if (!lasttag.empty()) {
...
bool is_open_tag(const string& text, size_t start, size_t stop) {
    ...
    while (pos < stop) {
        ...
        if (pos == string::npos || pos >= stop) return false;
        if ('/' == text[pos + 1]) {
            pos += 2;
            continue;
        }
        ...
        if (next == string::npos || next >= stop) return false;
        ...
        if (next == string::npos || next >= stop) return true;
    }
...
bool solve_query(const string& text, const string& query, string& result) {
    ...
    string lasttag;
    for (pos = query.find_first_of("~.");
         string::npos != pos && query[pos] == '.';
        ...
        if (in == string::npos) return false;
        in += tagged.length();

        if (!update_tag(text, in, tag, lasttag)) return false;
        if (is_open_tag(text, prev_in, in)) return false;
    }
    ...
    in = text.find(tagged, in);
    if (in == string::npos) return false;
    if (is_open_tag(text, prev, in)) return false;
    in += tagged.length();
    ...
    if (close_tag == string::npos) return false;
    ...
    prev = in;
    in = text.find(tagged, in);
    if (in == string::npos || in >= close_tag) return false;
    if (is_open_tag(text, prev, in + 1)) return false;
